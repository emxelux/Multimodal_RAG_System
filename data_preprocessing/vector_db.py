...
# Fixed: Use FastEmbedSparse to ensure compatibility with LangChain pipelines
...
def upsert_split_documents(markdown_nodes, user_id, source_name):
    docs_to_upsert = []
...
def retrieve_context(query, user_id, source_name, top_k=10):
...
if __name__ == "__main__":
    ...
    raw_results = retrieve_context(query_str, test_user, test_file, top_k=3)
    print(f"\nRetrieved {len(raw_results)} documents from Hybrid Search.")

    # 3. Apply Re-ranking via Cohere
    final_context = rerank_results(query_str, raw_results, top_n=3)
    print(f"\n--- Top {len(final_context)} Reranked Results ---")
    for idx, doc in enumerate(final_context):
        print(f"[{idx + 1}] (Score: {doc.metadata.get('relevance_score', 'N/A')}) -> {doc.page_content} Page: {doc.metadata.get("page")}...")